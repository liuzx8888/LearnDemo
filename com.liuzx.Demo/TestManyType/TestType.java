...
public class TestType {
	...
	public static void main(String[] agrs) {
		...
		/*
		 * 向下转型 子类引用 子类对象 = （子类引用）父类对象 B b1 = (B) new A(); Exception in thread "main"
		 * java.lang.ClassCastException: TestManyType.A cannot be cast to TestManyType.B
		 * at TestManyType.TestType.main(TestType.java:23)
		 *
		 * 向下转型 必须要先往上转型 ：父类不知道子类的类型，所以转换的时候报错 所以需要先把 B类向上转型，跟A类建立好关系后，再向下转型成B类才不报错
		 * 
		 *
		 */
		B b1 = (B) a2;
		...
		/*
		 * System.out.println(b1.show(a1)); System.out.println(b1.show(b));
		 * System.out.println(b1.show(c)); System.out.println(b1.show(d));
		 */
		...
		System.out.println(a2.show(b)); /*
										 * 多态，向上转型 a2代表 A类，先找 是否有对应的 show(b) 方法，A类没有该方法，虽然在B类有，
										 * B继承A，多态的特性必须继承，而且相同的方法被重写，才能调用子类方法，show(b)未被重写，所以 b转换成A类调用show (A obj)，同时show
										 * (A obj) 被重写，所以调用子类的方法 ：B AND A
										 */
